...
# schedule_year / schedule_month / saturday_dates 從 Tab1 解析後存入 session_state
...
with tab1:
    ...
    if uploaded_file:
        try:
            ...
            uploaded_file.seek(0)  # 重置指標，讓 Tab3 還能再讀

            # 掃描所有儲存格，收集所有 datetime
            found_dates = []
            ...
            if found_dates:
                ...
                st.warning("⚠️ 班表中找不到日期資料，請確認 Excel 格式是否正確（日期欄需為日期格式）。")

        except Exception as e:
            st.error(f"讀取班表失敗：{e}")

# ==========================================
# Tab 2: 助理劃休（依班表月份動態產生）
# ==========================================
with tab2:
    # 尚未上傳班表時提示
    ...
    days_in_month  = calendar.monthrange(year, month)[1]  # 當月實際天數
        ...
        if not uploaded_file:
            ...
            with st.spinner("正在解析班表並排班中..."):
                try:
                    ...
                    SAT_DATES = st.session_state.saturday_dates  # 從班表解析出的星期六
                    ...
                    for week_idx, week_start in enumerate(enlin_rows):
                        ...
                        for c in range(2, ws.max_column + 1, 2):
                            ...
                            v_next = ws.cell(date_row, c + 1).value
                            if isinstance(v, datetime.datetime):
                                day_num = v.day
                                # 以班表實際日期判斷是否星期六（不依賴寫死集合）
                                is_sat = (v.weekday() == 5)
                    ...
                    st.download_button(
                        "📥 下載排班結果",
                        output.getvalue(),
                        f"恩霖診所_{year}年{month}月排班結果.xlsx",
